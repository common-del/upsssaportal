...
export default async function DirectoryPage(props: {
  ...
  const withExtra = matches.map((s) => ({ school: s, extra: deriveResultFields(s.udise) }));
  const filtered = withExtra.filter(({ extra }) => {
    if (type && extra.type !== (type as SchoolType)) return false;
    if (performance && extra.performanceLevel !== (performance as PerformanceLevel)) return false;
    return true;
  });
            ...
            <tbody className="divide-y divide-border">
              {pageRows.map(({ school: s, extra }) => (
                <tr key={s.id} className="hover:bg-surface/60">
                  ...
                  <td className="hidden px-4 py-3 lg:table-cell">{hi ? s.block.nameHi : s.block.nameEn}</td>
                  <td className="px-4 py-3">{extra.type}</td>
                  <td className="px-4 py-3">
                    <span
